    ...
    namespace EAMain
    {
        namespace Internal
        {
            ...
            static void ReportDefault(const char8_t* pMessage)
            {
                ...
                else
                {
                        ...
                        OutputDebugStringA(reinterpret_cast<const char*>(pMessage));
                    #endif
                    ...
                    #else
                        fputs(reinterpret_cast<const char*>(pMessage), stdout);
                        fflush(stdout);
                    #endif
                }
            }
        ...
        static void ReportVaList(unsigned minVerbosity, ReportFunction pReportFunction, const char8_t* pFormat, va_list arguments)
        {
            if(pFormat && (GetVerbosity() >= minVerbosity))
            {
                ...
                const int nReturnValue = EA::StdC::Vsnprintf(buffer, kBufferSize, reinterpret_cast<const char*>(pFormat), arguments);
                ...
                if(pReportFunction)
                {
                    if((nReturnValue >= 0) && (nReturnValue < (int)kBufferSize))
                        pReportFunction(reinterpret_cast<const char8_t*>(buffer));
                    else if(nReturnValue < 0) // If we simply didn't have enough buffer space.
                    {
                        pReportFunction(reinterpret_cast<const char8_t*>("Invalid format specified.\n    Format: "));
                    ...
                    else // Else we simply didn't have enough buffer space.
                    {
                        ...
                        if(pBuffer)
                        {
                            ...
                            EA::StdC::Vsnprintf(pBuffer, nReturnValue + 1, reinterpret_cast<const char*>(pFormat), arguments);
                            pReportFunction(reinterpret_cast<const char8_t*>(pBuffer));
                            free(pBuffer);
                        }
                        else
                            pReportFunction(reinterpret_cast<const char8_t*>("Unable to allocate buffer space for large printf.\n"));
                    }
                }
