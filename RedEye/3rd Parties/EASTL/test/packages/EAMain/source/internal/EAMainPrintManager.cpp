...
#endif
    EA::EAMain::SetReportFunction(reinterpret_cast<EA::EAMain::ReportFunction>(EA::EAMain::Messages::Print));
    if (!printServerAddress)
    {
        printServerAddress = reinterpret_cast<const char8_t*>(MACRO_TO_STRING(EAMAIN_NETWORK_CHANNEL_IP));
    }
    const char8_t* server = printServerAddress;
    const char8_t* portStr = reinterpret_cast<const char8_t*>(strchr(reinterpret_cast<const char*>(printServerAddress), ':'));

    int port = 8080;//EAMAIN_NETWORK_CHANNEL_PORT;
    char8_t serverBuff[64] = { 0 };
    if (portStr != NULL)
    {
        port = EA::StdC::AtoI32(reinterpret_cast<const char*>(portStr + 1));
        server = reinterpret_cast<const char8_t*>(EA::StdC::Strncpy(reinterpret_cast<wchar_t*>(serverBuff), reinterpret_cast<const wchar_t*>(server), EA::StdC::Strlen(reinterpret_cast<const char*>(portStr - server))));
    }

    IChannel *networkChannel = Internal::CreateNetworkChannel(reinterpret_cast<const char*>(server), port);
    if (networkChannel)
    {
