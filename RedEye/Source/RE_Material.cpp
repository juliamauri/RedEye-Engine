...
void RE_Material::Draw()
{
	...
	if (RE_ResourceManager::TotalReferenceCount(GetMD5()) == 0)
	{
...
{
	metaNode->PushString("shaderMeta", (shaderMD5) ? RE_ResourceManager::At(shaderMD5)->GetMetaPath() : "NOMETAPATH");
...
void RE_Material::LoadResourceMeta(RE_Json* metaNode)
{
	...
	if (shaderMeta.compare("NOMETAPATH") != 0) shaderMD5 = RE_ResourceManager::FindMD5ByMETAPath(shaderMeta.c_str(), Resource_Type::R_SHADER);
...
{
	RE_Shader* matShader = dynamic_cast<RE_Shader*>(RE_ResourceManager::At(shaderMD5 ? shaderMD5 : RE_InternalResources::GetDefaultShader()));
	...
	{
		bool popAll = false;
		const char* selected = RE_ResourceManager::GetSelected();
		if (selected)
		{
			Resource_Type t = RE_ResourceManager::At(selected)->GetType();
			popAll = (t == Resource_Type::R_SHADER || t == Resource_Type::R_TEXTURE);
		}
		RE_ResourceManager::PushSelected(matShader->GetMD5(), popAll);
	}
	...
	{
		eastl::vector<ResourceContainer*> shaders = RE_ResourceManager::GetResourcesByType(Resource_Type::R_SHADER);
		bool none = true;
		for (auto  shader :  shaders)
		{
			...
			{
				if (ResourceContainer::inMemory && shaderMD5) RE_ResourceManager::UnUse(shaderMD5);
				shaderMD5 = shader->GetMD5();
				if (ResourceContainer::inMemory && shaderMD5) RE_ResourceManager::Use(shaderMD5);
				GetAndProcessUniformsFromShader();
				applySave = true;
			}
		}
	...
	if (usingOnMat[CDIFFUSE] || usingOnMat[TDIFFUSE])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tDiffuse.back());
				applySave = true;
			}
	...
	if (usingOnMat[CSPECULAR] || usingOnMat[TSPECULAR])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tSpecular.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[CAMBIENT] || usingOnMat[TAMBIENT])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tAmbient.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[CEMISSIVE] || usingOnMat[TEMISSIVE])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			...
			{
				tAmbient.push_back(*static_cast<const char**>(dropped->Data));
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tEmissive.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[OPACITY] || usingOnMat[TOPACITY])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tOpacity.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[SHININESS] || usingOnMat[SHININESSSTRENGHT] || usingOnMat[TSHININESS])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tShininess.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[THEIGHT])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tHeight.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[TNORMALS])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tNormals.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (usingOnMat[TREFLECTION])
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(tReflection.back());
				applySave = true;
			}
			ImGui::EndDragDropTarget();
		}
	...
	if (ImGui::BeginDragDropTarget())
	{
		if (const ImGuiPayload* dropped = ImGui::AcceptDragDropPayload("#TextureReference"))
		{
			...
			if (ResourceContainer::inMemory) RE_ResourceManager::Use(tUnknown.back());
			applySave = true;
		}
		ImGui::EndDragDropTarget();
	}
...
void RE_Material::DrawTextures(const char* texturesName, eastl::vector<const char*>* textures)
{
	...
	if (!textures->empty())
	{
		...
		{
			ResourceContainer* resource = RE_ResourceManager::At(*md5);
			if (ImGui::Button(eastl::string("Texture #" + eastl::to_string(count) + " " + eastl::string(resource->GetName())).c_str()))
			{
				...
				RE_ResourceManager::PushSelected(*md5, popAll);
			}
			...
			{
				if (ResourceContainer::inMemory) RE_ResourceManager::UnUse(*md5);
				toDelete = md5;
				applySave = true;
			}
			...
			if (ImGui::BeginMenu(changetexture.c_str()))
			{
				...
				for (auto textRes : allTex)
				{
					if (ImGui::MenuItem(textRes->GetName()))
					{
						if (ResourceContainer::inMemory) RE_ResourceManager::UnUse(*md5);
						*md5 = textRes->GetMD5();
						if (ResourceContainer::inMemory) RE_ResourceManager::Use(*md5);
						applySave = true;
					}
				}
				ImGui::EndMenu();
	...
	if (ImGui::BeginMenu(((addtexture += texturesName) += " texture").c_str()))
	{
		...
		for (auto textRes : allTex)
		{
			if (ImGui::MenuItem(textRes->GetName()))
			{
				...
				if (ResourceContainer::inMemory) RE_ResourceManager::Use(textures->back());
				applySave = true;
			}
		}
		ImGui::EndMenu();
...
{
	Config material(GetAssetPath(), RE_FileSystem::GetZipPath());
	if (material.Load()) {
		...
		if (size)
		{
			...
			for (uint i = 0; i < size; i++)
			{
				...
				case RE_Cvar::SAMPLER: sVar.SetSampler(RE_ResourceManager::IsReference(nuniforms->PullString(id.c_str(),"")), true); break;
				}
...
void RE_Material::PullTexturesJson(RE_Json * texturesNode, eastl::vector<const char*>* textures)
{
	...
	for (uint i = 0; i < texturesSize; i++)
	{
		...
		const char* textureMD5 = RE_ResourceManager::FindMD5ByMETAPath(textureMaT.c_str());
...
{
	Config materialSerialize(GetAssetPath(), RE_FileSystem::GetZipPath());
...
void RE_Material::PushTexturesJson(RE_Json * texturesNode, eastl::vector<const char*>* textures)
{
	...
	for (uint i = 0; i < texturesSize; i++)
	{
		eastl::string idref = "MetaPath";
		idref += eastl::to_string(i).c_str();
		texturesNode->PushString(idref.c_str(), RE_ResourceManager::At(textures->operator[](i))->GetMetaPath());
	}
}
		...
		if (uSize > 0)
		{
			for (uint i = 0; i < uSize; i++)
			{
				...
				case RE_Cvar::SAMPLER:
				{
					...
					{
						size = sizeof(char) * nSize;
						sVar.SetSampler(RE_ResourceManager::IsReference(eastl::string(cursor, nSize).c_str()), true);
						cursor += size;
					}
...
{
	if(shaderMD5) RE_ResourceManager::Use(shaderMD5);
	for (auto t : tDiffuse) RE_ResourceManager::Use(t);
	for (auto t : tSpecular) RE_ResourceManager::Use(t);
	for (auto t : tAmbient) RE_ResourceManager::Use(t);
	for (auto t : tEmissive) RE_ResourceManager::Use(t);
	for (auto t : tOpacity) RE_ResourceManager::Use(t);
	for (auto t : tShininess) RE_ResourceManager::Use(t);
	for (auto t : tHeight) RE_ResourceManager::Use(t);
	for (auto t : tNormals) RE_ResourceManager::Use(t);
	for (auto t : tReflection) RE_ResourceManager::Use(t);
	for (auto t : tUnknown) RE_ResourceManager::Use(t);
}

void RE_Material::UnUseResources()
{
	if (shaderMD5) RE_ResourceManager::UnUse(shaderMD5);
	for (auto t : tDiffuse) RE_ResourceManager::UnUse(t);
	for (auto t : tSpecular) RE_ResourceManager::UnUse(t);
	for (auto t : tAmbient) RE_ResourceManager::UnUse(t);
	for (auto t : tEmissive) RE_ResourceManager::UnUse(t);
	for (auto t : tOpacity) RE_ResourceManager::UnUse(t);
	for (auto t : tShininess) RE_ResourceManager::UnUse(t);
	for (auto t : tHeight) RE_ResourceManager::UnUse(t);
	for (auto t : tNormals) RE_ResourceManager::UnUse(t);
	for (auto t : tReflection) RE_ResourceManager::UnUse(t);
	for (auto t : tUnknown) RE_ResourceManager::UnUse(t);
}
...
void RE_Material::UploadToShader(const float* model, bool usingChekers, bool defaultShader)
{
	...
	RE_Shader* shaderRes = dynamic_cast<RE_Shader*>(RE_ResourceManager::At(usingShader));
	...
	else if (usingChekers)
	{
		...
		RE_GLCacheManager::ChangeTextureBind(RE_InternalResources::GetTextureChecker());
	}
	else
	{
		if (usingOnMat[CDIFFUSE])			RE_ShaderImporter::setFloat(ShaderID, "cdiffuse", cDiffuse);
		...
		for (unsigned int i = 0; i < tDiffuse.size() || i < usingOnMat[TDIFFUSE]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tDiffuse[i]))->use();
		}
		for (unsigned int i = 0; i < tSpecular.size() || i < usingOnMat[TSPECULAR]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tSpecular[i]))->use();
		}
		for (unsigned int i = 0; i < tAmbient.size() || i < usingOnMat[TAMBIENT]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tAmbient[i]))->use();
		}
		for (unsigned int i = 0; i < tEmissive.size() || i < usingOnMat[TEMISSIVE]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tEmissive[i]))->use();
		}
		for (unsigned int i = 0; i < tOpacity.size() || i < usingOnMat[TOPACITY]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tOpacity[i]))->use();
		}
		for (unsigned int i = 0; i < tShininess.size() || i < usingOnMat[TSHININESS]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tShininess[i]))->use();
		}
		for (unsigned int i = 0; i < tHeight.size() || i < usingOnMat[THEIGHT]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tHeight[i]))->use();
		}
		for (unsigned int i = 0; i < tNormals.size() || i < usingOnMat[TNORMALS]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tNormals[i]))->use();
		}
		for (unsigned int i = 0; i < tReflection.size() || i < usingOnMat[TREFLECTION]; i++)
		{
			...
			dynamic_cast<RE_Texture*>(RE_ResourceManager::At(tReflection[i]))->use();
		}
	}
	...
	for (uint i = 0; i < fromShaderCustomUniforms.size(); i++)
	{
		...
		case RE_Cvar::SAMPLER:
		{
			if (fromShaderCustomUniforms[i].AsCharP())
			{
				...
				dynamic_cast<RE_Texture*>(RE_ResourceManager::At(fromShaderCustomUniforms[i].AsCharP()))->use();
			}
			break;
		}
		}
...
unsigned int RE_Material::GetShaderID() const
{
	...
	return dynamic_cast<RE_Shader*>(RE_ResourceManager::At(usingShader))->GetID();
}
...
void RE_Material::GetAndProcessUniformsFromShader()
{
	...
	eastl::vector<RE_Shader_Cvar> fromShader = dynamic_cast<RE_Shader*>(RE_ResourceManager::At(shaderMD5 ? shaderMD5 : RE_InternalResources::GetDefaultShader()))->GetUniformValues();
