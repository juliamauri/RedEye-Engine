...
void RE_GameObject::SetUp(ComponentsPool* compPool, const char* _name, RE_GameObject* parent, bool start_active, bool _isStatic)
{
	active = start_active;
	isStatic = _isStatic;
	name = _name;
	poolComponents = compPool;
	(transform = dynamic_cast<RE_CompTransform*>(poolComponents->GetNewComponent(ComponentType::C_TRANSFORM)))->SetUp(this);

	if (parent) parent->AddChild(this, false);
...
void RE_GameObject::DrawWithChilds() const
{
	if (active)
	{
		...
		while (!gos.empty())
		{
			...
			for (auto component : go->GetComponents()) component->Draw();
...
eastl::list<RE_Component*> RE_GameObject::GetComponents() const
{
	...
	for (auto compid : componentsID) ret.push_back(poolComponents->GetComponent(compid.poolId, static_cast<ComponentType>(compid.cType)));
	return ret;
}
...
eastl::vector<RE_GameObject*> RE_GameObject::GetActiveChildsWithDrawComponents()
{
	...
	if (active)
	{
		...
		while (!go_queue.empty())
		{
			...
			go_queue.pop();
			if (go->HasDrawComponents()) ret.push_back(go);
...
bool RE_GameObject::HasDrawComponents() const
{
	for (auto component : GetComponents())
	{
		...
		if (cT == ComponentType::C_MESH || (cT > ComponentType::C_PRIMIVE_MIN && cT < ComponentType::C_PRIMIVE_MAX)) return true;
	}
...
void RE_GameObject::DeserializeBinary(char*& cursor, ComponentsPool* compPool, eastl::map<int, RE_GameObject*>* idGO)
{
	...
	else SetUp(compPool, strName, nullptr);
...
void RE_GameObject::AddChild(RE_GameObject * child, bool broadcast)
{
	...
	if (broadcast) Event::Push(GO_HAS_NEW_CHILD, App::scene, this, child);
}
...
void RE_GameObject::AddChildsFromGO(RE_GameObject * go, bool broadcast)
{
	...
	// TODO Rub: Add or remove broadcast paramater
...
const eastl::list<RE_GameObject*>& RE_GameObject::GetChilds() const { return childs; }

void RE_GameObject::GetChilds(eastl::list<const RE_GameObject*>& out_childs) const
{
	for (auto child : childs) out_childs.push_back(child);
}

unsigned int RE_GameObject::ChildCount() const { return childs.size(); }
bool RE_GameObject::IsLastChild() const { return parent && (parent->childs.back() == this); }
...
void RE_GameObject::SetActive(const bool value, const bool broadcast)
{
	...
	{
		active = value;
		if (broadcast) Event::Push(active ? GO_CHANGED_TO_ACTIVE : GO_CHANGED_TO_INACTIVE, App::scene, this);
	}
}
...
{
	active = value;
	for (auto child : childs) child->SetActiveRecursive(active);
}
...
void RE_GameObject::SetStatic(const bool value, const bool broadcast)
{
	...
	{
		isStatic = value;
		if (active && broadcast) Event::Push(isStatic ? GO_CHANGED_TO_STATIC : GO_CHANGED_TO_NON_STATIC, App::scene, this);
	}
}
...
{
	RE_CompCamera* comp_camera = dynamic_cast<RE_CompCamera*>(poolComponents->GetNewComponent(ComponentType::C_CAMERA));
...
RE_CompPrimitive* RE_GameObject::AddCompPrimitive(ushortint type, int _slices, int _stacks, float _radius)
{
	...
	ret = (RE_CompPrimitive * )poolComponents->GetNewComponent(_type);
	...
	case C_TETRAHEDRON:
	case C_OCTOHEDRON:
	case C_ICOSAHEDRON: App::primitives.SetUpComponentPrimitive(ret, this); break;
	case C_PLANE:
	case C_SPHERE:
	case C_CYLINDER:
	case C_HEMISHPERE:
	case C_TORUS:
	case C_TREFOILKNOT: dynamic_cast<RE_CompParametric*>(ret)->SetUp(this, dynamic_cast<RE_Shader*>(App::resources->At(App::internalResources.GetDefaultShader()))->GetID(), _slices, _stacks, true, _radius); break; }
...
{
	RE_CompPrimitive* ret = nullptr;
	ret = dynamic_cast<RE_CompPrimitive*>(poolComponents->GetNewComponent(C_ROCK));
	dynamic_cast<RE_CompRock*>(ret)->SetUp(this, (dynamic_cast<RE_Shader*>(App::resources->At(App::internalResources.GetDefaultShader())))->GetID(), _rockSeed, _rockNSubdivvisions);
	return ret;
}
...
RE_Component* RE_GameObject::AddComponent(const ushortint type)
{
	...
	case C_MESH:
	{
		RE_CompMesh* mesh_comp = dynamic_cast<RE_CompMesh*>(poolComponents->GetNewComponent(ComponentType::C_MESH));
		mesh_comp->SetUp(this);
		ret = static_cast<RE_Component*>(mesh_comp);
		break;
	}
	case C_CAMERA:
	{
		...
		ret = static_cast<RE_Component*>(comp_camera);
		break; 
	}
	case C_LIGHT:
	{
		RE_CompLight* comp_light = dynamic_cast<RE_CompLight*>(poolComponents->GetNewComponent(ComponentType::C_LIGHT));
		comp_light->SetUp(this);
		ret = static_cast<RE_Component*>(comp_light);
		break; 
	}
	case C_PARTICLEEMITER:
	{
		ret = static_cast<RE_Component*>(new RE_CompParticleEmitter(this));
		break;
	}
	case C_GRID:
	{
	...
	case C_ROCK:
	{
		if (ret = poolComponents->GetNewComponent(_type))
			App::primitives.SetUpComponentPrimitive(dynamic_cast<RE_CompPrimitive*>(ret), this);
	...
	if (!ret) RE_LOG_ERROR("GameObject could not add type %u component", type);
...
{
	return dynamic_cast<RE_CompMesh*>(poolComponents->GetNewComponent(ComponentType::C_MESH));
}
...
RE_CompMesh * RE_GameObject::GetMesh() const
{
	...
	for (auto component : GetComponents())
	{
		...
		{
			ret = dynamic_cast<RE_CompMesh*>(component);
			break;
		}
	}
...
RE_CompCamera * RE_GameObject::GetCamera() const
{
	...
	for (auto component : GetComponents())
	{
		...
		{
			ret = dynamic_cast<RE_CompCamera*>(component);
			break;
		}
	}
...
RE_CompLight* RE_GameObject::GetLight() const
{
	...
	for (auto component : GetComponents())
	{
		...
		{
			ret = dynamic_cast<RE_CompLight*>(component);
			break;
		}
	}
...
void RE_GameObject::TransformModified(bool broadcast)
{
	...
	else Event::Push(TRANSFORM_MODIFIED, App::scene, this);
}

const char * RE_GameObject::GetName() const { return name.c_str(); }

void RE_GameObject::AddToBoundingBox(math::AABB box)
{
	if (transform) local_bounding_box.Enclose(box);
}
...
math::AABB RE_GameObject::GetGlobalBoundingBoxWithChilds() const
{
	...
	if (HasDrawComponents()) ret = global_bounding_box;
...
void RE_GameObject::ResetLocalBoundingBox()
{
	...
	{
		switch (comp->GetType()) {
		case C_MESH:		 local_bounding_box.Enclose((dynamic_cast<RE_CompMesh*>(comp))->GetAABB()); break;
...
void RE_GameObject::ResetBoundingBoxForAllChilds()
{
	...
	while (!go_queue.empty())
	{
		...
		for (auto child : go->childs) go_queue.push(child);
	}
}
...
void RE_GameObject::DrawProperties()
{
	...
	{
		// TOFO Julius: if (ImGui::MenuItem("Save as prefab")) {}
		if (ImGui::MenuItem("Activate Childs")) IterativeSetActive(true);
		if (ImGui::MenuItem("Deactivate Childs")) IterativeSetActive(false);
		if (ImGui::MenuItem("Childs to Static")) IterativeSetStatic(true);
		if (ImGui::MenuItem("Childs to non-Static")) IterativeSetStatic(false);
		ImGui::EndMenu();
	}
